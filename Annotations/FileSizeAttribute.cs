...
namespace Vizitz.Annotations
{
    ...
    public sealed class FileSizeAttribute : ValidationAttribute
    {
        ...
        public override string FormatErrorMessage(string name)
        {
            ...
            string errorMessage = MinimumSize != 0 ? "minimum size is zero" : ErrorMessageString;
        ...
        private void EnsureLegalSizes()
        {
            if (MaximumSize < 0)
            {
                throw new InvalidOperationException("maximum size is less than zero");
            }

            if (MaximumSize < MinimumSize)
            {
                throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, "minimum size is greater than maximum size", MaximumSize, MinimumSize));
            }
        }
    }
}
